...
On replay, accepted output is read without relaunching its assignment or re-normalizing its bytes. Unaccepted output can be accepted only after the original worker settles and cleans. Crash recovery adopts the durable Paseo, Herdr, tmux or direct handle; it never overwrites that record by starting another worker with the same action ID. A timed-out writer blocks replacement while its handle is retained or cleanup is unproven. A late durable exit-status file, or a live Herdr `done` observation matching the original pane, allows cleanup. Only then may invalid/absent output follow the existing replacement limit. A genuine subsequent action has independent output **and log** paths. These lifecycle safety fixes do not upgrade policy versions or reset allowances.
