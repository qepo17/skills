...
On replay, every outstanding intent is reconciled, including pending actions with recorded workers. Pending does not mean never launched. An interrupted later packet is settled before the implementation phase gates a new packet on predecessor checks; its partial writes can legitimately stale that earlier source snapshot. New packet admission still requires passing current checks.

On replay, accepted output is read without relaunching its assignment or re-normalizing its bytes. Unaccepted output can be accepted only after the original worker settles and cleans. Crash recovery adopts the durable Paseo, Herdr, tmux or direct handle; it never overwrites that record by starting another worker with the same action ID. A timed-out writer blocks replacement while its handle is retained or cleanup is unproven. Such a block preserves the outstanding action and its lease/one-shot claim for later adoption; it does not discard the only link to a surviving verifier. Missing adoption for a recorded worker is unknown cleanup, never an inferred success. Ordinary batch acceptance also requires successful cleanup. A late durable exit-status file, or a live Herdr `done` observation matching the original pane, allows cleanup. Only then may invalid/absent output follow the existing replacement limit. A genuine subsequent action has independent output **and log** paths. These lifecycle safety fixes do not upgrade policy versions or reset allowances.
...
Initialization validates intake types/question history before creating run state and copies it into the existing hash-pinned `requirements.json`. Canonical worker inputs already carry that artifact, so no separate research worker, source-artifact bundle, or ticket phase is needed. Planners inspect the relevant code themselves and synthesize settled context into the existing plan fields: source-linked problem/solution and meaningful stories, implementation/testing decisions, domain glossary/ADRs, non-goals, and baseline-bound steps/files/validation links. They select the highest practical existing behavioral test seams with prior art, and form independently verifiable vertical packets with genuine dependencies. Necessary prefactors and wide-refactor expand–contract patterns remain within current risk, packet, and validation limits; this adds no integration-branch mechanism or multi-repository writer. No separate spec artifact, story quota, routine seam confirmation, or breakdown-approval quiz is required. Reviewers compare the original source and the implementation plan, including story coverage, test behavior, domain decisions, and dependency correctness. Workers do not interview the user or publish tracker changes.
...
**New Paseo Codex launches are blocked**, including bootstrap preflight, dry-run previews, and the actual launcher. Paseo's [provider configuration takes precedence over mode presets](https://github.com/getpaseo/paseo/blob/main/packages/server/src/server/agent/providers/codex-app-server-agent.ts): `auto` alone cannot prove sandboxing or approval behavior. The supported [run CLI](https://github.com/getpaseo/paseo/blob/main/packages/cli/src/commands/agent/run.ts) submits the initial prompt during creation, while [inspect](https://github.com/getpaseo/paseo/blob/main/packages/cli/src/commands/agent/inspect.ts) reports the mode, not effective permissions. Neither a parent inspection, a mode label, nor an operator's configuration claim proves a new child's effective policy before its prompt executes. No launch or exploratory prompt is sent when that proof is unavailable.

Paseo Pi remains supported. For Codex, choose direct, tmux, or Herdr execution for a separately authorized new run in the appropriate active environment; do not silently fall back, switch runtime, or edit an existing run's pinned backend. Existing Paseo Codex handles may still be adopted, waited on, and archived without a new prompt; recovery does not retrofit their permissions or permit replacement launches. Restoring Paseo Codex launch support requires a backend-enforced permission contract or a prompt-free creation and effective-policy verification path, not another mode-name check.
...
- `validation-retry/retry-later-interrupted` requires separate explicit user authorization after a successful original retry, all approved packets accepted, and a different current implementation tree that pins the prior verifier. Only one required `not-run`/unknown-exit observation with a pinned partial log is eligible. The graph uses `later_validation_retry_attempts` without altering the original claim; neither may be replenished. The exact interruption attestation and old overbroad-gate handling are in [schemas/run-amendment.md](schemas/run-amendment.md). It grants no source writes, waiver, or privilege.
...
### Lost later-packet intent after host reboot

The opt-in `recover-interrupted-packet` command handles only the [documented initializer/stale-predecessor condition](schemas/interrupted-packet-recovery.md). It requires explicit authorization, exact source/history hashes, the next approved packet's unchanged scope, remaining replacement budget, cleaned original identity, separate user confirmation of local-only origin without relocation, and positive Linux/systemd host-reboot evidence. Legacy timestamps alone cannot establish worker host identity. Fresh Herdr/process observations must rule out restored task workers. It preserves old artifacts and queues a new attempt through the existing implement node, without accepting the initializer, resetting limits or bypassing any check/review/delivery gate. `--no-drive` applies the guarded projection only; ordinary resume then reconciles it. Identical requests are idempotent. Protected-process permission errors, missing cwd on a surviving process, and PID-reuse/inspection uncertainty remain blocked before intent/state writes. The descriptor-bound scan never exempts a process by name, ancestry or startup timing. Only the explicitly authorized [one-time privileged read-only inspector](schemas/privileged-process-inspection.md) may acquire unavailable metadata. Its exact source/nonce/request/host/original-UID proof and consumed attempt are hash-pinned; neither elevated orchestration, automatic sudo retry nor caller-supplied clearance is supported. This does not broaden ordinary resume or recover arbitrary failed/unfinished results.

### Overlapping writer / overwritten accepted blocked output

After separate explicit authorization to repair the engine and recover the incident, `recover-writer-incident` can quarantine the one unavailable reference and schedule fresh read-only verification. Read [the exact admission, hash, cleanup, and preservation contract](schemas/writer-incident-recovery.md). This narrow transition requires a single-repository policy-version-1 Herdr run, attempts 1/2 of the same approved packet, immutable retained-timeout proof, a late complete candidate, and an overwritten infrastructure-blocked replacement with no executed checks. All other evidence must validate.

The command validates every unaffected historical assignment/check-log binding before cleanup. It proves the request-pinned original Pi session from the task-bound launch prefix and pre-timeout lifetime, and closes only an exclusively owned single-pane/tab workspace with no other occupant. It preserves raw state/output/history and the original missing hash, and never accepts either incident output or rewrites its bytes. The existing implement node independently verifies the combined source and runs original packet checks freshly in a new read-only action. All remaining packets, full-plan checks, review, delivery and budgets remain intact. `--no-drive` permits validated status inspection before graph execution; it does not touch the pending SQLite cursor. Ordinary `resume` cannot perform this quarantine.
