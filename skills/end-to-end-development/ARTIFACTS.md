...
A failed, settled advisory-text repair with no output can be finished by `complete-handoff-metadata`, without another worker. It requires the exact missing-output execution rejection and an intact complete original result with only oversized text. The existing `artifact_repairs` entry retains its launch claim and may add `metadata_completion` with a hashed assignment, `started_at`, and eventually `completed_at`. This resumable command creates only the missing output, setting `next_action` to null and validating exact semantic equivalence. It never overwrites an existing result, classifies blockers, resets allowances, or accepts changed repository/evidence state. Other failed repairs stay blocked. New derived assignments select validator/schema paths from the running skill.

### Coordinator whitespace validation

`stage: validate` may use `execution_mode: command` only with a hash-pinned `coordinator_validation_source` result, a `coordinator_repository_state` (content fingerprint, HEAD, branch, semantic index SHA-256), and no project/Git/forge worker permissions. The graph selects this only when the exact planned `git diff --check` is the sole non-passing record, marked `not-run` with null exit code, and all remaining evidence matches the current plan and repository. It executes fixed, hardened Git argv rather than plan text. The new result retains prior passing records as hash-referenced `reused` evidence and records the actual whitespace outcome as `fresh`. A failed check is not converted to a pass, source results are immutable, and retry limits/approvals are unchanged. No agent handle is created; command intent, manifest and crash reconciliation use the existing command path.
