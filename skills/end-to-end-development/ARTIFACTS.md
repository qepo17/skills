...
- `validation_retry_attempts`: at most one immutable validate-assignment reference per repository/run, claimed before an explicitly authorized interrupted-validation verifier launch. Its assignment pins the recorded retry amendment. Reconciliation can accept the original settled output but cannot replenish or replace the claim;
