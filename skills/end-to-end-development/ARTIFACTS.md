...
The user-authorized `repair-handoff-metadata` command handles only an oversized advisory `next_action` on an unaccepted completed implementation result. It preserves the rejected bytes at `recoveries/handoff-<original-sha256>.original.json`, writes a hash-pinned receipt beside them, and replaces only `next_action` with null at the original assigned output path. Every other semantic field and validation result is retained. The engine checks original artifact identity, approved input hashes, current worktree/Git evidence, passing checks and settled worker cleanup before writing. The original assignment and already accepted artifacts never change. `run.json.handoff_metadata_recoveries[action_id]` points to the receipt; the original action is accepted normally and the exact blocker is cleared without changing retry limits or skipping graph phases. A crash after publication can complete from the same immutable archive. This is an explicit command recovery, not an agent repair attempt or a general schema waiver.
