...
- `pending_validation_refresh`: at most one hashed restoration or interrupted-validation retry amendment per repository, consumed by validation-only work. Neither pending map adds a graph phase or resets a source-fix budget;
- `validation_retry_attempts`: at most one immutable validate-assignment reference per repository/run, claimed before an explicitly authorized original interrupted-validation verifier launch. Its assignment pins the recorded `retry-interrupted` amendment. Reconciliation can accept the original settled output but cannot replenish or replace the claim;
- `later_validation_retry_attempts`: a separate, at-most-one validate-assignment reference per repository/run for `retry-later-interrupted`. This requires a successful consumed original retry and a distinct accepted final implementation packet with one hash-pinned incomplete execution. The original claim stays unchanged; both claim maps enforce their exact decision type;
...
With `validation_policy_version: 1`, validation IDs and commands are built together from sorted effective check objects. Excluded checks are absent from future executable lists; the authorizing amendment remains a hash-pinned assignment input. `validation-fix` participates in exact ID/command coverage. A remediation assignment adds `remediation` and separately identifies its repair targets in `failed_validation_ids`, while rerunning the full effective suite. A restoration or interrupted-validation refresh adds `validation_refresh`. Interrupted retries pin a `validation-retry` amendment, require fresh evidence for its selected IDs, and may reuse other passing checks only from its pinned current source result. They preserve source/HEAD/branch/index and never receive project/Git/forge write permission. The assignment tells the worker to report complete source/check-reporting work as complete even when a command fails, honor exclusions, disclose advisory failures, and use its unique log paths.
