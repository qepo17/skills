...
Read [DISCOVERY.md](DISCOVERY.md) for source intake, evidence-first decisions, and the task-wide question cap. Existing tickets skip ticket creation; `idea-to-ticket` is optional upstream preparation. The graph's existing `plan` phase synthesizes the implementation spec in canonical plan artifacts: problem/solution, meaningful stories, implementation/testing decisions, domain/ADR context, and internally dependency-linked vertical slices. It adapts `to-spec` and `to-tickets` without another spec interview, exhaustive story quota, or breakdown-approval quiz. Do not add a second spec/ticket pipeline or a separate orchestrator.
...
2. Resolve answerable questions from code/docs and adopt supported, reversible, in-scope recommendations without routine confirmation. Ask only material unresolved questions under [DISCOVERY.md](DISCOVERY.md): zero when clear, otherwise one at a time (up to three only when independent), at most 10 total including upstream questions and follow-ups. Preserve the ledger in the bootstrap `intake`; do not initialize with unresolved material choices. A later full-profile approval gate remains mandatory and cannot be replaced by a recommendation.
...
For a settled, accepted validation-fix result with its source-fix allowance exhausted and exactly one required command blocked by reviewed enclosing-harness interruption evidence, the documented `validation-retry/retry-interrupted` amendment adds one read-only verifier per repository/run, not a source-fix allowance. It requires explicit user authorization and pinned source/harness logs; a recorded 124 alone does not prove interruption. Use `amend ... --no-drive`, inspect, then ordinary `resume`. The graph owns the one-shot claim and assignment. Selected exact commands must run freshly with individual timeouts; unchanged other passing checks can be reused only from the pinned current source. Preserve failed evidence, immutable approval, all budgets and every protected check. See [schemas/run-amendment.md](schemas/run-amendment.md) for the exact additional `interruption` field and fail-closed conditions.

A later interruption in a distinct final accepted implementation packet can use `validation-retry/retry-later-interrupted` only with **separate explicit authorization**, a successful accepted original verifier, unchanged approved meaning, and a pinned partial log with unknown exits. The graph claims one separate later verifier and preserves the earlier consumed claim. This is not a repeat allowance for failed verification, an ordinary resume bypass, or permission to reset any source-fix limit. Read the exact schema before preparing evidence; no further retries are automatic.
...
`--worker-runtime auto` inherits a Paseo parent's Pi/Codex provider when present, otherwise follows the coordinator: Codex when `CODEX_THREAD_ID` is present and Pi by default. **New Paseo Codex launches are blocked** because the supported CLI cannot prove effective sandbox/approval permissions before submitting the prompt; mode presets can be overridden. Paseo Pi and direct/tmux/Herdr Codex launches remain supported. Existing Paseo Codex handles can still settle and clean up, but must not be replaced or silently moved to another backend/runtime. See the [worker permission boundary](ORCHESTRATION.md#worker-permission-boundary). `E2E_COORDINATOR_RUNTIME` remains an internal diagnostic override, not required user setup.
