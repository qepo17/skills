...
## Read scope and write permissions

`repositories[].access: read` authorizes reading that repository's instructions, source, and tests for the assigned work. The legacy `project_file_access`, `git_access`, and `forge_access` fields are **write-permission switches**: `none` denies writes, not scoped read-only inspection. Thus a planner with `project_file_access: none` and a repository `access: read` must inspect current project files without editing them. This interpretation also preserves existing immutable assignments; do not rewrite them or add a new permission value. Runtime sandbox denials remain blockers, and read scope never permits unrelated repository access or overrides artifact-repair restrictions on rerunning checks.

An assignment with a hash-pinned `generation_recovery` may additionally contain `generated_file_writes`: exact absolute generated output paths authorized by the [producer-order recovery](generation-recovery.md). This is a narrow exception to the project write switch, including check-only stages, not permission to edit other files in a read-only producer. `generation_source_state` pins producer source/Git evidence; changing it fails acceptance. Runtime sandbox controls remain unchanged. Without this recovery authority, no generated-file write exception exists.
