...
A coordinator-owned whitespace check uses `stage: validate`, `execution_mode: command`, and a hashed `coordinator_validation_source`. All worker permissions remain `none`. The graph, not a worker, executes only the fixed safe Git whitespace command, reusing other passing source records without editing them. This is not permission to execute arbitrary validation text, Git writes, or forge commands.
