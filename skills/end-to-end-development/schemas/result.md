...
- Decisions have an ID, `kind`, summary, and evidence. The only permitted `kind` values are `implementation`, `validation`, `pipeline`, `finding-resolution`, and `bounded-plan-deviation`. For validation environment/cache decisions, use `validation`; do not invent subtypes such as `validation-environment`. Use `bounded-plan-deviation` only for a change that preserves requirements/contract, adds no mechanism, follows repository precedent, and stays within the packet concern.
- `next_action` must be a nonempty string of at most 300 characters. Keep supporting detail in the existing evidence/logs, not in this routing hint.
