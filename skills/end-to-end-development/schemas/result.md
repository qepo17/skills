...
For `execution_mode: artifact-repair`, initialization copies the original result. By default repair only missing existing blocker classifications from the pinned evidence. When `repair_of.kind` is `next-action-length`, initialization already replaces the overlong `next_action` with a fixed reference to its hash-pinned original text: validate that output and return, without rewriting its handoff or classifying blockers. Keep all other semantic fields unchanged, including status, outcomes, validations (including failed checks), blocker text, and IDs. Do not run tests or write project/Git/forge state. The graph pins the original assignment/output, evidence files, content, HEAD, branch, and index; any mutation or failed repair blocks rather than starting another source writer.
...
- `next_action` is a nullable informational handoff of at most **300 characters**, not workflow routing authority. Put detailed instructions in logs and reference them briefly; LangGraph determines the next phase.
