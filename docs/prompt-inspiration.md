...
Historical scope: these notes describe the durable engine and idea-to-ticket planning resources. Ordinary development now follows the concise [development guide](../skills/end-to-end-development/DEVELOPMENT.md); it does not load these references or inherit durable stage gates. The fast skill no longer bundles the older discovery and plan-writing guides.
...
Planning presentation also draws on the installed `show-me/SKILL.md`, read in full (SHA-256 `bea6da70a58096730b9aeb0bae293ddf4726103a98efc9ce13c481619942a810`). Its contribution is the smallest useful view, short adjacent prose, focused trees/flows/diffs, and restraint—not compulsory diagrams or HTML. Durable mode bundles the resulting [plan-writing guide](../skills/end-to-end-development/PLAN-WRITING.md). This is a local source fingerprint, not a claim that an upstream repository snapshot was verified.
...
The engine test checks that source-inspired synthesis/testing/slicing/review guidance reaches actual immutable worker assignments while stage counts and existing gates remain unchanged. Packaging checks validate standalone skills and matching coordinator development/delivery resources. The plan-writing guide's example and pre-presentation review support qualitative readability checks; the automated suite does not grade writing quality. The idea skill's bundled examples support read-only behavioral smoke cases. Neither substring assertions nor mocked workers prove a model's semantic coverage or question counting; record such evaluations separately and do not claim live tracker publication was tested.
